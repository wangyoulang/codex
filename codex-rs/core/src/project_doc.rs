...
mod tests {
    ...
    async fn skills_are_appended_to_project_doc() {
        ...
        let expected = format!(
            "base doc\n\n## Skills\nA skill is a local instruction bundle rooted at a `SKILL.md` file. Each entry below includes a name, description, and file path so you can open the source for full instructions when using a specific skill.\n### Available skills\n- pdf-processing: extract from pdfs (file: {expected_path_str})\n### How to use skills\n{usage_rules}"
        );
        assert_eq!(res, expected);
    }
